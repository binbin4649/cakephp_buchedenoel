<?php
class AmountDepotsController extends AppController {

	var $name = 'AmountDepots';
	var $helpers = array('Html', 'Form');
	var $uses = array('AmountDepot', 'Depot');
	...
	function index() {
		...
		$this->AmountDepot->recursive = 0;
		$this->set('amountDepots', $this->paginate());
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid AmountDepot.', true));
			$this->redirect(array('action'=>'index'));
		}
		$this->set('amountDepot', $this->AmountDepot->read(null, $id));
	}
	...
	function add() {
		...
		if (!empty($this->data)) {
			...
			if ($this->AmountDepot->save($this->data)) {
				$this->Session->setFlash(__('The AmountDepot has been saved', true));
				$this->redirect(array('action'=>'index'));
			} else {
				$this->Session->setFlash(__('The AmountDepot could not be saved. Please, try again.', true));
			}
		}
	}
	...
	function edit($id = null) {
		...
		if (empty($this->data)) {
			$this->data = $this->AmountDepot->read(null, $id);
		}
	}

	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for AmountDepot', true));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->AmountDepot->del($id)) {
			$this->Session->setFlash(__('AmountDepot deleted', true));
			$this->redirect(array('action'=>'index'));
		}
	}
