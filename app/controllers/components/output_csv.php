...
class OutputCsvComponent extends Object {
	...
	function InventoryDetail($values){
		$depot_total = 0;
		$real_total = 0;
		$out = '棚卸番号,明細番号,倉庫名,倉庫番号,スパン,フェイス,子品番,JAN,帳簿数,実棚数,差異,入力者,入力日時,更新者,更新日時'."\r\n";
		foreach($values as $value){
			...
			$out .= $InventoryDetail['depot_quantity'] - $InventoryDetail['qty'].',';
	...
	//棚卸明細 一致したのは外すバージョン
	function InventoryDetail2($values){
		$depot_total = 0;
		$real_total = 0;
		$out = '棚卸番号,明細番号,倉庫名,倉庫番号,スパン,フェイス,子品番,JAN,帳簿数,実棚数,差異,入力者,入力日時,更新者,更新日時'."\r\n";
		foreach($values as $value){
			extract($value);// InventoryDetail   Depot   Subitem
			if($InventoryDetail['depot_quantity'] != $InventoryDetail['qty']){
				$depot_total = $depot_total + $InventoryDetail['depot_quantity'];
				$real_total = $real_total + $InventoryDetail['qty'];
				$out .= $InventoryDetail['inventory_id'].',';
				$out .= $InventoryDetail['id'].',';
