<?php
class AmountDestination extends AppModel {

	var $name = 'AmountDestination';
			...
			foreach($amount_type as $key=>$value){
				...
				$params = array(
					'conditions'=>array(
						...
						$ModelName.'.destination_id'=>$destination['Destination']['id']
					),
					'recursive'=>0
				);
				...
				if($Amount){
					...
					$Amount = array();
					$Amount[$ModelName]['name'] = $destination['Destination']['name'].'-'.$destination['Destination']['id'].$value.$start_day.'-'.$end_day;
					$Amount[$ModelName]['amount_type'] = $key;
					$Amount[$ModelName]['destination_id'] = $destination['Destination']['id'];
