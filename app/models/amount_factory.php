<?php
class AmountFactory extends AppModel {

	var $name = 'AmountFactory';
		...
		foreach($amount_type as $key=>$value){
			...
			$params = array(
				'conditions'=>array(
					...
					$ModelName.'.factory_id'=>$item['Item']['factory_id']
				),
				'recursive'=>0
			);
			...
			if($Amount){
				...
				$Amount = array();
				$Amount[$ModelName]['name'] = $item['Factory']['name'].'-'.$item['Item']['factory_id'].$value.$start_day.'-'.$end_day;
				$Amount[$ModelName]['amount_type'] = $key;
				$Amount[$ModelName]['factory_id'] = $item['Item']['factory_id'];
