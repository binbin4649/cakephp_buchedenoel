<div class="amountItemproperties form">
<?php echo $form->create('AmountItemproperty');?>
	<fieldset>
 		<legend><?php __('Edit AmountItemproperty');?></legend>
	<?php
		...
		echo $form->input($sub_id, array(
			'type'=>'select',
			'options'=>$itemproperty,
			'div'=>true,
			'label'=>__($subModel, true),
		));
...
<div class="actions">
	<ul>
		<li><?php echo $html->link(__('Delete', true), array('action'=>'delete', $form->value('AmountItemproperty.id')), null, sprintf(__('Are you sure you want to delete # %s?', true), $form->value('AmountItemproperty.id'))); ?></li>
