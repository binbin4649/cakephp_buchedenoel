...
foreach($ranking_bests as $value){
		echo '<tr>';
		echo '<td>';
		echo $html->link($value['ladys_name'], array('controller'=>'items', 'action'=>'view/'.$value['ladys_id']));
		echo ' | ';
		echo $html->link($value['mens_name'], array('controller'=>'items', 'action'=>'view/'.$value['mens_id']));
		echo '</td>';
