...
<div class="form">
	...
	echo $contact4['User']['name'];
 	echo '</td></tr><tr><td>';
 	echo '　<a href="/'.SITE_DIR.'/destinations/selectid?keepThis=true&TB_iframe=true&height=400&width=550" title="出荷先選択" class="thickbox">出荷先</a>';
	//echo '　<a href="/'.SITE_DIR.'/destinations" target="_blank">出荷先</a>：';
	...
	echo $form->hidden('OrderDateil.order_status', array('value'=>0));
	...
	if(!empty($details)){
		...
		foreach($details as $key=>$value){
			...
			if($value['Subitem']['order_type'] <> 4){
				echo $html->link('Edit', array('action'=>'add/edit/'.$key)).' | ';
			}
			echo $html->link('Del', array('action'=>'add/del/'.$key)).'</td>';
			echo '</tr>';
		}
